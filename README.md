...
6. Підтримка мульти-акаунтів для скрапінгу різних Instagram акаунтів

### Підтримка мульти-акаунтів

Система підтримує скрапінг та відображення даних з різних Instagram акаунтів. Кожен акаунт має:
- Свою власну базу даних SQLite
- Окрему папку для зберігання зображень
- Незалежну статистику та відображення у веб-інтерфейсі

#### Чому окремі бази даних?

Ми використовуємо окремі бази даних SQLite для кожного акаунту з наступних причин:

1. **Ізоляція даних** - дані різних акаунтів повністю ізольовані, що зменшує ризик помилкового змішування даних
2. **Продуктивність** - менші бази даних працюють швидше, особливо при великій кількості постів
3. **Переносимість** - окремі файли баз даних легше переносити, резервувати та відновлювати
4. **Масштабованість** - можна легко додавати нові акаунти без зміни структури існуючих баз даних

#### Як працює мульти-акаунт підтримка

1. **Налаштування акаунтів**:
   - Акаунти налаштовуються в файлі `accounts_config.py`
   - Кожен акаунт має унікальне ім'я користувача, назву бази даних та папку для зображень

2. **Скрапінг та парсинг**:
   - При запуску скрапінгу ви вибираєте акаунт з випадаючого списку
   - Скрапер використовує відповідну базу даних та папку для зберігання даних
   - Усі пости зберігаються з позначкою акаунту

3. **Веб-інтерфейс**:
   - На головній сторінці та сторінці постів є випадаючий список для вибору акаунту
   - При зміні акаунту відображаються дані тільки для цього акаунту
...
Для налаштування мульти-акаунтів відредагуйте файл `accounts_config.py`. Цей файл містить список акаунтів та їх налаштування:
...
Щоб додати новий акаунт:

1. Додайте новий запис до списку `AVAILABLE_ACCOUNTS`
2. Запустіть скрипт `create_account_db.py` для створення нової бази даних
...
### Конфігурація URL для скрапінгу

Система використовує JSON файл для зберігання URL-адрес, які будуть парситись. Це дозволяє легко змінювати URL через веб-інтерфейс без редагування файлів конфігурації кожного разу.

Файл `url_config.json` створюється автоматично і має наступну структуру:

```json
{
    "url_posts": "https://www.instagram.com/ваш_акаунт/",
    "url_reels": "https://www.instagram.com/ваш_акаунт/reels"
}
```

Існує два способи налаштувати URL для скрапінгу:

1. **Через веб-інтерфейс**: Відкрийте сторінку налаштувань (/settings) та заповніть поля URL.

2. **Через редагування JSON файлу**: Змініть значення в файлі `url_config.json`.

> **Примітка**: Якщо URL не встановлені, система покаже попередження і заблокує запуск скрапінгу.
